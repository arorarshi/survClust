...
We see how logrank statistic is maximized at **k=3** and Standardized Pooled Withink Sum of Square Statistic (SPSS) is elbow-ed at k=3 as well. 
...
Where, 
`x` = a list of matrices of data types with samples as rows and features as columns. 
`survdat` = a matrix of two columns where first column is the reported time and second column are the events. Rows are samples. 
