...
To run supervised integrative clustering analysis - we will be calling `cv.survclust`. Cross-validation takes time and we will be using the `BiocParallel` package and splitting cross-validation across `k` clusters. 
...
uvm_all_cvrounds<-function(kk){
    ...
    fit<-list()
    for (i in 1:10){
        fit[[i]] <- cv.survclust(uvm_dat,uvm_survdat,kk,this.fold)
...
sim_cvrounds<-function(kk){
    ...
    fit<-list()
    for (i in 1:10){
        fit[[i]] <- cv.survclust(simdat, simsurvdat,kk,this.fold)
        print(paste0("finished ", i, " rounds for k= ", kk))
    }
    return(fit)
}
...
cvrounds_mut <- function(kk){
    ...
    fit<-list()
    for (i in 1:10){
        fit[[i]] <- cv.survclust(uvm_mut_dat, uvm_survdat,kk,this.fold, type="mut")
        print(paste0("finished ", i, " rounds for k= ", kk))
    }
    return(fit)
}
