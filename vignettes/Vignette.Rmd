...
We will use it to see the distribution of these mutations across the 4 clusters. 

```{r, fig.cap= "TCGA UVM mutation features for k=4"}

sort_idx <- order(k4)

panelmap::makepanel(k4[sort_idx], 
                    gr.name = "k4", gr.col = c("black","red", "blue", "green"),
...
```{r}
ptm <- Sys.time()
sim_cv.fit<-foreach(kk=2:7) %dopar% cvrounds(simdat, simsurvdat,kk)
ptm2 <- Sys.time()
...
The output is a list object consisting of 6 sub-lists for $k = 2:7$, with 10 `cv.survclust` output (for each round of cross-validation), each consisting of `cv.labels`, `cv.logrank`, `cv.spwss` for 3 folds. 
...
#function to do cross validation 
cvrounds_mut <- function(x,survdat,kk){
    this.fold<-3
    fit<-list()
    for (i in 1:10){
        fit[[i]] <- cv.survclust(x,survdat,kk,this.fold, type="mut")
        print(paste0("finished ", i, " rounds for k= ", kk))
    }
    return(fit)
}
...
ptm <- Sys.time()
uvm_mut_cv.fit<-foreach(kk=2:7) %dopar% cvrounds_mut(uvm_mut_dat, uvm_survdat,kk)
ptm2 <- Sys.time()
...
```{r, fig.cap="TCGA UVM mutation data alone for k=4"}

sort_idx <- order(k4)

panelmap::makepanel(k4[sort_idx], 
                    gr.name = "k4", gr.col = c("red", "blue", "green", "lightblue"),
                    uvm_dat[[1]][sort_idx,c("SF3B1","GNAQ", "GNA11", "CYSLTR2")], 
                    mat.col = list(GNAQ = c("white", "black"), GNA11 = c("white", "black"), SF3B1 = c("white", "black"), CYSLTR2 = c("white", "black")), mat.type = c(1,1,1,1))
